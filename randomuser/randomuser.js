...
window.onload = () => {
	...
	document.getElementById("btnGetRandomUser").addEventListener("click", get_users);
    ...
    document.getElementById("btnSaveXML").addEventListener("click", save_users_xml);
};
...
function get_users() {
	...
	fetch("https://randomuser.me/api/?nat=es")
        ...
        resultado.innerHTML = "";
        resultado.innerHTML = "Usuario cargado correctamente";
}
...
function generate_users_table() {
    ...
    resultado.innerHTML = "";
    resultado.innerHTML = "Usuario añadido a tabla correctamente";
}
...
function save_users() {
	...
	fetch("save_users.php", {
		...
		.then((data) => {
            ...
            resultado.innerHTML = "";
            resultado.innerHTML = "Usuario insertado correctamente Fetch";
		})
		.catch((error) => {
...
function save_users_xml() {
    ...
    let usuarios_a_enviar = JSON.stringify(users_to_save);

    xhr.onreadystatechange = () => {
        if (xhr.readyState === 4 && xhr.status === 200) {
            resultado.innerHTML = "";
            resultado.innerHTML = "Usuario insertado correctamente XML";
        }
    }
    xhr.send(usuarios_a_enviar);
}